...
func (s *Sequencer) runTimer() {
	...
	for {
		...
		s.xmitCond.L.Lock()
		s.debugf("Sequencer.runTimer: len(s.rxmtBuf)=%d seqXmt=%d seqRxmt=%d seqRcv=%d\n",
		...
		s.debugf("Sent retransmit of seq %d ack %d s.seqXmt %d s.seqRxmt %d s.seqRcv %d len(s.rxmtBuf) %d\n",
...
func (s *Sequencer) backgroundRead() {
	...
	for {
		...
		s.debugf("s.c.Read(readbuf()) returned len %d err %s\n", nn,
			err)
		if err != nil || nn < 8 {
			if err == nil {
		...
		if err != nil {
			s.debugf("Exiting backgroundRead: error reading flags: %s\n",
				err)
			return
		}
		err = binary.Read(buf, binary.LittleEndian, &seq)
		if err != nil {
			s.debugf("Exiting backgroundRead: error reading seq: %s\n",
				err)
			return
		}
		err = binary.Read(buf, binary.LittleEndian, &ack)
		if err != nil {
			s.debugf("Exiting backgroundRead: error reading ack: %s\n",
				err)
			return
		}
		err = binary.Read(buf, binary.LittleEndian, &msgLen)
		if err != nil {
			s.debugf("Exiting backgroundRead: error reading msgLen: %s\n",
				err)
			return
		}
		s.debugf("backgroundRead: seq %d ack %d len %d seqRxmt %d seqRcv %d\n",
			seq, ack, msgLen, s.seqRxmt, s.seqRcv)

		s.xmitCond.L.Lock()
		uDistanceRxmt := ack - s.seqRxmt
		distanceRxmt := int16(uDistanceRxmt)
		s.debugf("backgroundRead: ack is %d seq is %d uDistanceRxmt is %d distanceRxmt is %d s.seqRxmt %d s.seqXmt %d\n",
			ack, seq, uDistanceRxmt, distanceRxmt, s.seqRxmt, s.seqXmt)
		if distanceRxmt >= 0 {
			if distanceRxmt > int16(len(s.rxmtBuf)) {
				s.debugf("Exiting BackgroundRead: distanceRxmt (%d) > len(s.rxmtBuf) (%d)\n",
					distanceRxmt,
					int16(len(s.rxmtBuf)))
				return
			}
			...
			s.debugf("Updated received sequence by %d: len(s.rxmtBuf) = %d s.seqRxmt = %d\n",
		...
		s.debugf("distanceSeq is %d seq is %d s.seqRcv is %d len(dataBuf) is %d\n",
			distanceSeq, seq, s.seqRcv, len(dataBuf))
		if distanceSeq >= 0 {
			if distanceSeq <= int16(len(dataBuf)) {
				s.debugf("Databuf before shrink: %s\n",
					dataBuf)
				dataBuf = dataBuf[distanceSeq:]
				s.debugf("Databuf after shrink: %s\n",
					dataBuf)
			}
			s.debugf("backgroundRead: s.recvBuf %s databuf %s\n",
				s.recvBuf, dataBuf)
			s.lastRcv = time.Now()
...
func (s *Sequencer) Read(p []byte) (n int, err error) {
	for {
		...
		for {
			n = len(s.recvBuf)
			s.debugf("Sequencer.Read: len(p)=%d len(s.recvBuf) %d\n",
				len(p), n)
			if n != 0 {
				break
			}
		...
		s.debugf("Sequencer.Read: len(s.recvBuf) %d s.recvBuf %s\n",
			len(s.recvBuf), s.recvBuf)
		copy(p, s.recvBuf)
		s.recvBuf = s.recvBuf[n:]
		s.recvCond.L.Unlock()
		s.debugf("Returning Sequencer.Read: len(s.recvBuf) %d\n",
			len(s.recvBuf))
		return n, nil
	}
}
...
func (s *Sequencer) Write(p []byte) (n int, err error) {
	...
	s.xmitCond.L.Lock()
	s.debugf("Sequencer.Write: len(s.rxmtBuf)=%d len(p)=%d seqXmt=%d seqRxmt=%d seqRcv=%d\n",
...
func (s *Sequencer) debugf(f string, args ...interface{}) {
	//	fmt.Printf(f, args...)
}
